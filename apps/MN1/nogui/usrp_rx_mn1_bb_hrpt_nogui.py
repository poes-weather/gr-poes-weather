...
# Generated: Wed Dec 15 21:21:20 2010
##################################################
...
class usrp_rx_mn1_bb_hrpt_nogui(gr.top_block):

	def __init__(self, decim=32, gain=35, side="A", sync_check=False, satellite='METEOR-M-1', frames_file=os.environ['HOME'] + '/METEOR-M-1.hrpt', baseband_file=os.environ['HOME'] + '/METEOR-M-1.dat', freq=1700e6):
		gr.top_block.__init__(self, "USRP Meteor M N1 HRPT Receiver")
...
if __name__ == '__main__':
	...
	tb = usrp_rx_mn1_bb_hrpt_nogui(decim=options.decim, gain=options.gain, side=options.side, sync_check=options.sync_check, satellite=options.satellite, frames_file=options.frames_file, baseband_file=options.baseband_file, freq=options.freq)
	tb.start()
