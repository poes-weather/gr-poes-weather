...
# Generated: Wed Jun 22 12:15:23 2011
##################################################
...
class usrp_rx_noaa_bb_hrpt(grc_wxgui.top_block_gui):
	...
	def __init__(self, satellite='NOAA-XX', baseband_file=os.environ['HOME'] + '/NOAA-XX.dat', frames_file=os.environ['HOME'] + '/NOAA-XX.hrpt', side="A", gain=35, freq=1698e6, decim=32, sync_check=False):
	...
	def get_baseband_outfile_text(self):
		return self.baseband_outfile_text
...
if __name__ == '__main__':
	...
	tb = usrp_rx_noaa_bb_hrpt(satellite=options.satellite, baseband_file=options.baseband_file, frames_file=options.frames_file, side=options.side, gain=options.gain, freq=options.freq, decim=options.decim, sync_check=options.sync_check)
	tb.Run(True)
